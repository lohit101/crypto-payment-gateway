import { ArrowLeftRight, Bell, ChevronDown, Download, MessageCircle, MoveDownLeft, MoveUpRight, Wallet, Receipt, ChevronRight, MoreHorizontal, ChartNoAxesCombined, FileChartColumn, Link, Copy, QrCode, SquareArrowOutUpRight, ExternalLink, FileText, WalletMinimal, HandCoins } from "lucide-react";
...
export default function HeadDashboard() {
  ...
  const getLinks = async () => {
    try {
      const response = await fetch("/api/public/getlinks", {
        method: "GET",
        headers: { "Content-Type": "application/json" },
      });
    ...
    <div className="min-h-screen bg-white">
      <header className="bg-black text-white p-8">
        <div className="container mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-4 gap-5">
            <Logo />
            <nav className="flex-row items-center justify-center gap-3 hidden md:flex space-x-4">
              <button className="flex flex-row gap-1 items-center justify-center">Buy Crypto <ChevronDown className="ml-1 h-4 w-4" /></button>
              <button className="flex flex-row gap-1 items-center justify-center">Trade <ChevronDown className="ml-1 h-4 w-4" /></button>
              <button className="flex flex-row gap-1 items-center justify-center">Payments <ChevronDown className="ml-1 h-4 w-4" /></button>
              <button className="flex flex-row gap-1 items-center justify-center">More <ChevronDown className="ml-1 h-4 w-4" /></button>
      ...
      <nav className="hidden sm:flex bg-white border-b shadow-[0_35px_30px_-15px_rgba(0,0,0,0.05)] sticky top-0 z-50">
        <div className="container mx-auto flex space-x-8 py-5 w-full md:w-3/4">
          <button className="text-black font-semibold border-b border-b-black">Overview</button>
          <button className="text-gray-500 border-b border-transparent hover:text-gray-800 hover:border-b-black transition-all">Personal</button>
          <button className="text-gray-500 border-b border-transparent hover:text-gray-800 hover:border-b-black transition-all">Business</button>
          <button className="text-gray-500 border-b border-transparent hover:text-gray-800 hover:border-b-black transition-all">P2P</button>
          <button className="text-gray-500 border-b border-transparent hover:text-gray-800 hover:border-b-black transition-all">History</button>
          <button className="text-gray-500 border-b border-transparent hover:text-gray-800 hover:border-b-black transition-all">Settings</button>
        </div>
      </nav>
      ...
      <main className="container mx-auto p-4 w-full md:w-3/4">
        <div className="bg-zinc-100 rounded-3xl p-6 mt-10">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <AccountCard title="Personal" amount={personalBalance ? personalBalance : <div className="flex bg-gray-200 rounded-lg h-8 w-16 animate-pulse"></div>} badge="" />
            <AccountCard title="Business" amount={businessBalance ? businessBalance : <div className="flex bg-gray-200 rounded-lg h-8 w-16 animate-pulse"></div>} badge="+$0.00" />
      ...
      <div className="container mx-auto p-4 w-full md:w-3/4 mt-10">
        ...
        <div className="w-full mt-5">
                      ...
                      <div className="dropdown-menu w-72">
                        <label htmlFor="modal-1" className="dropdown-item text-sm">
                          <div className="flex flex-col">
                            ...
                            <p className="text-zinc-600">Change wallet address to recieve your money in.</p>
                          </div>
                        </label>
                        <a tabIndex={-1} className="dropdown-item text-sm" href="/withdraw">
                          <div className="flex flex-col">
                            ...
                            <p className="text-zinc-600">Withdraw your balance to your wallet.</p>
                          </div>
                        </a>
                      </div>
                    </div>
          ...
          <div>
            <div className="text-center py-8 bg-gray-100 rounded-3xl mt-10">
              <h2 className="text-xl font-medium mb-2">No transactions yet</h2>
              <p className="text-gray-500 mb-4">You don't have any transactions</p>
              <button className="btn bg-black hover:bg-zinc-800 transition-all text-white">Get address</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-5">
          <div className="rounded-lg w-full md:w-2/5 h-max">
            <div className="flex flex-col items-start gap-3">
              ...
              <button className="btn bg-black text-white py-2 px-8 rounded-xl text-xs hover:bg-black/80 transition-all" onClick={(e) => createPaymentLink(e.target)}>Create Link</button>
              {paymentLink && (
                <div className="">
                      ...
                      <div className="dropdown-menu">
                        <a className="dropdown-item text-sm" href={link.paymentUrl} target="_blank">
                          <div className="flex flex-col">
                            ...
                            <p className="text-zinc-600">Visit your payment link</p>
                          </div>
                        </a>
                        <a tabIndex={-1} className="dropdown-item text-sm">
                          <div className="flex flex-col" onClick={() => navigator.clipboard.writeText(link.paymentUrl)}>
                            ...
                            <p className="text-zinc-600">Copy your payment link to clipboard</p>
                          </div>
                        </a>
                      </div>
                    </div>
    ...
    <div className="bg-white p-4 rounded-2xl hover:shadow-lg transition-all">
      <div className="flex justify-between items-center">
        ...
        <div className="dropdown">
          ...
          <div className="dropdown-menu w-72">
            <a className="dropdown-item text-sm" href={title === "Personal" ? "/analytics/personal" : title === "Business" ? "/analytics/business" : null}>
              <div className="flex flex-col">
                <p className="flex flex-row gap-1 items-center font-medium"><ChartNoAxesCombined size={16} />Analytics</p>
                <p className="text-zinc-600">See how well you've performed in terms of numbers.</p>
              </div>
            </a>
            <a tabIndex={-1} className="dropdown-item text-sm" href={title === "Personal" ? "/reports/personal" : title === "Business" ? "/reports/business" : null}>
              <div className="flex flex-col">
                <p className="flex flex-row gap-1 items-center font-medium"><FileChartColumn size={16} />Report</p>
                <p className="text-zinc-600">Get a report of all your transactions.</p>
              </div>
            </a>
            {title === 'Business' ?
              <a tabIndex={-1} className="dropdown-item text-sm" href={title === "Personal" ? "/reports/personal" : title === "Business" ? "/reports/business" : null}>
                <div className="flex flex-col">
                  ...
                  <p className="text-zinc-600">Learn how to integrate CryptoWall in your website.</p>
                </div>
              </a> : null
            }
          </div>
